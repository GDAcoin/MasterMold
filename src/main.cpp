...
uint256 hashGenesisBlock("0x9ca97fdf3c4000110b4b65b4888ea093e75ed8bd6eda4a722becf9dccf05bdd5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Molds: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MasterMold Signed Message:\n";
...
{
    // Molds: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    if(nHeight == 2)
    {
    nSubsidy = 77399 * COIN; //premine ==> 2,58703933402789% [77400 coins]
    }
    else if(nHeight < 103)      
    {
    nSubsidy = 1 * COIN; //miners setup == 100 blocs x1 coin
    }
    else if(nHeight < 20104)      
    {
    nSubsidy = 40 * COIN; //20000 blocks x40 ==> 800,000 MOLDS
    }
     else if(nHeight < 100105) 
    {
    nSubsidy = 10 * COIN; //80000 blocks x10 ==> 800,000 MOLDS
    }
     else if(nHeight < 200106) 
    {
    nSubsidy = 4 * COIN; //100000 blocks ==> 400,000 MOLDS
    }
     else if(nHeight < 300107) 
    {
    nSubsidy = 2 * COIN; //100000 blocks ==> 200,000 MOLDS
    } 
    else if(nHeight < 301108) 
    {
    nSubsidy = 1 * COIN; //1000 blocks ==> 1,000 MOLDS
    }
    else if(nHeight < 301109) 
    {
    nSubsidy = 1337 * COIN; //"EXTRA LUCKY BLOCK" == 1337 MOLDS
    }
    else if(nHeight < 302110) 
    {
    nSubsidy = 10 * COIN; //1000 blocks x10 ==> 10000 MOLDS
    }
    else if(nHeight < 303111) 
    {
    nSubsidy = 2 * COIN; //1000 blocks x10 ==> 2000 MOLDS
    }
    // 302110 blocs = MINER's TIME ==> approx. 3 mining months [supply = 2,291,837 coins] 
    else if(nHeight < 1303112) 
    {
    nSubsidy = 0.7 * COIN; //1,000,000 blocks x0.1 ==> 700,000 MOLDS
    }
    return nSubsidy + nFees;
    // 1M blocs = ENTHOUSIAST PEOPLE's TIME ==> approx. 10 years mining [supply = 700,000 coins] 
    // setgenerate true=1 [wallet mining]
    // if success ==> miners will come back :-)
    //[Totalpanda supply :-) = 2,991,837 coins] 
}

static const int64 nTargetTimespan = 5 * 30; // MasterMold : diff retargeting every 5 blocks
static const int64 nTargetSpacing = 1 * 30; // MasterMold : 30s block ==> 288 block per day
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Molds: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Molds: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Molds: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
//1425881551
        hashGenesisBlock = uint256("0x9ca97fdf3c4000110b4b65b4888ea093e75ed8bd6eda4a722becf9dccf05bdd5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Tue, 05 May 2015 02:34:54 GMT ==> lead-1-usa-feu-vert-a-la-fusion-entre-holcim-et-lafarge";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xf3f68b84a4c01b07f42fd985790b8ff8ce8a8a2e5ca6af5a988cb8dca15c48df"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MasterMold: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MasterMoldMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MasterMoldMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MasterMoldMiner(CWallet *pwallet)
{
    printf("MasterMoldMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mastermold-miner");
    ...
    try { loop {
        ...
        printf("Running MasterMold with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MasterMoldMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MasterMoldMiner, pwallet));
}
